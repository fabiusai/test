...
function generaReport() {
  ...
  reader.onload = function (e) {
    ...
    if (filtered.length) {
      ...
      output.textContent = "Impossibile generare il report.";
    }
  };
...
function esportaCSVFormattato() {
  ...
  for (const label in grouped) {
    ...
    for (const argomento in argomenti) {
      ...
      row["Totale"] = ["Facebook", "Instagram", "LinkedIn", "Twitter", "YouTube"].map(k => (typeof row[k] === "number" ? row[k] : 0)).reduce((a, b) => a + b, 0);
